...
def process_module(module_info_addr):
    ...
    print "ent_top: {} ent_len: {} stub_top: {} stub_len: {}".format(hex(ent_top), hex(ent_len), hex(stub_top), hex(stub_len))

    exports = []
    base_addr = addr = module_info_addr + INFO_SIZE
    while addr - base_addr < ent_len:
...
def find_strings():
    ...
    while seg_start != BADADDR:
        ...
        if not bytes:
            continue
        print "Got seg bytes: {}".format(len(bytes))
    ...
    while addr != BADADDR:
        ...
        if GetMnem(addr) == "MOV":
            ...
            for x in range(16):
                ...
                if GetMnem(next_addr) in ["B", "BX", "BL", "BLX"]:
                    break
                if GetMnem(next_addr) == "MOVT.W" and GetOpnd(next_addr, 0) == reg:
                    if GetOpnd(next_addr, 1)[0] == "#":
                        found = True
                        val += GetOperandValue(next_addr, 1) * (2 ** 16)
                    break
